...
LOG = '/mnt/filsystem1/code/dsb2017/code/ZJJ/3d_resnet/kaggle_3d.txt'
NODULE_NUM = '/mnt/filsystem1/code/dsb2017/code/ZJJ/3d_resnet/nodule_num.txt'
...
if __name__=='__main__':
    ...
    # patient_names = df_train.id
    # patient_labels = df_train.cancer
    ...
    for order, patient in enumerate(patients):
        if os.path.isfile(SAVE_FOLDER + patient + '.npy'):
            ...
            f = open(NODULE_NUM, 'a')
            text = str(patient)+' nodule_numbers:'+str(len(nodule_centers))+'\n'
            f.writelines(text)
            f.close()
            ...
            for i, coords in enumerate(nodule_centers):
                ...
                if ((xy_slice.shape != (OUTPUT_DIM, OUTPUT_DIM)) or (xz_slice.shape != (OUTPUT_DIM, OUTPUT_DIM)) or (yz_slice.shape != (OUTPUT_DIM, OUTPUT_DIM))):
                    f = open(LOG,'a')
                    content = str(patient) + '\n'
                    content += 'xy.shape:'+str(xy_slice.shape)+'\n'
                    content += 'xz.shape:'+str(xz_slice.shape)+'\n'
                    content += 'yz.shape:'+str(yz_slice.shape)+'\n'
