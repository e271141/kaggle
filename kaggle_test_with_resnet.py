...
CANCER_FOLDER = '/mnt/filsystem1/code/dsb2017/code/ZJJ/3d_resnet/cancer/'
CANCER_TXT = '/mnt/filsystem1/code/dsb2017/code/ZJJ/3d_resnet/cancer.txt'
NODULE_TXT = '/mnt/filsystem1/code/dsb2017/code/ZJJ/3d_resnet/prob.txt'
        ...
        if not res.values:
            ...
            for i, coords in enumerate(nodule_centers):
                ...
                if ((xy_slice.shape == (OUTPUT_DIM, OUTPUT_DIM)) and (xz_slice.shape == (OUTPUT_DIM, OUTPUT_DIM)) and (yz_slice.shape == (OUTPUT_DIM, OUTPUT_DIM))):
                    ...
                    accuraccy.append([xy_acc, xz_acc, yz_acc])
            ...
            if len(probability)>0:
                ...
                np.save(SAVE_FOLDER+patient+'.npy', prob)
                print SAVE_FOLDER+patient+'.npy  saved!'
                result.append([patient, max_prob])
                contet = patient + ':' + str(max_prob)
            else:
                result.append([patient, 0])
                content = patient + ':' + '0'
            fout.writelines(content+'\n')
            fout.close()

            fout = open(CANCER_TXT, 'a') 
            if len(cancer)>0:
                prob = np.array(cancer, dtype='float32')
                max_prob = np.amax(prob)
                print prob.shape, max_prob
                np.save(CANCER_FOLDER+patient+'.npy', prob)
                print CANCER_FOLDER+patient+'.npy  saved!'
                result1.append([patient, max_prob])
                contet = patient + ':' + str(max_prob)
            else:
                result1.append([patient, 0])
                contet = patient + ':' + '0'
            fout.writelines(content+'\n')
            fout.close()
