...
SAVE_FOLDER = '/mnt/filsystem1/code/dsb2017/code/ZJJ/3d_resnet/probability_0.8_threshold/'
SAVE_FOLDER_CANCER = '/mnt/filsystem1/code/dsb2017/code/ZJJ/3d_resnet/cancer_0.8_threshold/'
    ...
    for order, patient in enumerate(patients):
        ...
        if not res.values:
            ...
            if len(cancer)>0:
                ...
                max_prob = np.amax(prob)
                print prob.shape, max_prob
                np.save(SAVE_FOLDER_CANCER+patient+'.npy', prob)
                print SAVE_FOLDER_CANCER+patient+'.npy  saved!'
                result1.append([patient, max_prob])
            else:
                result1.append([patient, 0])

    df = pd.DataFrame(result,columns=PREDICTIONS)
    df.to_csv('0.8_threshold_result.csv', index=False)

    df1 = pd.DataFrame(result1,columns=PREDICTIONS)
    df1.to_csv('0.8_threshold_result1.csv', index=False)
