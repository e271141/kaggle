...
PROB_FOLDER = THD_PREPROCESS + 'probability_0.8/'
CANCER_FOLDER = THD_PREPROCESS + 'cancer_0.8/'
ACCURACY_FOLDER = THD_PREPROCESS + 'accuracy_0.8/'
CANCER_TXT = THD_PREPROCESS + 'cancer_0.8.txt'
NODULE_TXT = THD_PREPROCESS + 'nodule_0.8.txt'
ACCURACY_TXT = THD_PREPROCESS + 'accuracy_0.8.txt'
...
if __name__=='__main__':
    ...
    for u_input in os.listdir(UNET_INPUT):
        ...
        if os.path.exists(THD_PREPROCESS+'cancer_0.8/'+u_input):
            temp = np.load(THD_PREPROCESS+'probability_0.8/'+u_input)
            ctemp = np.load(THD_PREPROCESS + 'cancer_0.8/'+u_input)
            ...
            # f_prob.writelines(save_name+':'+prob_temp_max+'\n')
            # f_cancer.writelines(save_name+':'+cancer_ctemp_max+'\n')
            # f_prob.close()
            # f_cancer.close()
            continue
